...
class Annotations(object):
    ...
    def remove_in_time(self, coordinate, class_name, tpl):
        plate = coordinate.plate
        position = coordinate.position
        time = coordinate.time
        ann=self._annotations
        items = ann[plate][position][time][class_name]
        length = len(items)
        if length == 1:
            for t in ann[plate][position]:
                if(int(t)>=int(time)):
                    try : 
                        del ann[plate][position][t][class_name]
                    except KeyError:
                        pass

        else : 
#            item = items[class_name]
            self.remove(coordinate, tpl)# : pourquoi ca ne marche pas ?
...
class TrackingModule(Module):
    ...
    def __init__(self, parent, browser, settings, imagecontainer):
        ...
        self._trajectories = Annotations()
        self._object_items = {}

        splitter = QSplitter(Qt.Vertical, self)

        grp_box = QGroupBox('Cells', splitter)
        ...
        self._import_class_definitions_btn = QPushButton('Import track colours')
        ...
        grp_box = QGroupBox('Trajectories', splitter)
        layout = QBoxLayout(QBoxLayout.TopToBottom, grp_box)
        layout.setContentsMargins(5, 10, 5, 5)
    ...
    def _on_class_apply(self):
        ...
        if not class_name is None:
            ...
            if len(class_name_new) == 0:
                warning(self, "Invalid cell name",
                        info="The cell name must not be empty!")
                ...
                self._trajectories.rename_class(class_name, class_name_new)
                self._current_class = class_name_new
                self._activate_objects_for_image(False)
                self._activate_objects_for_image(True)
            else:
                warning(self, "Cell names and labels must be unique!",
                        info="Cell name '%s' or label '%s' already used." %\
    ...
    def _on_class_remove(self):
        ...
        if (not class_name is None and
            question(self, "Do you really want to remove class '%s'?" % \
                     ...
                     self._trajectories.get_count_for_class(class_name))):
            ...
            self._trajectories.remove_class(class_name)
    ...
    def _init_new_classifier(self):
        ...
        self._class_text.setText('cell1')
    ...
    def _on_open_classifier(self):
        ...
        if result:
            ...
            if not learner is None:
                ...
                try:
                    has_invalid = self._trajectories.import_from_xml(path2,
            ...
            if self._save_classifier(result):
                try:
                    ...
                    for filename in filenames:
                        ...
                        os.remove(filename)
                    self._trajectories.export_to_xml(path2,
    ...
    def _activate_objects_for_image(self, state, clear=False):
        ...
        for class_name, tpl in self._trajectories.iter_items(coordinate):
            point = QPointF(*tpl)
    ...
    def _update_class_table(self):
        ...
        '''
        counts = self._trajectories.get_class_counts()
    ...
    def _update_annotation_table(self):
        ...
        '''
        per_class = \
            self._trajectories.get_annotations_per_class(self._current_class)
        ann_table = self._ann_table
    ...
    def _on_new_point(self, point, button, modifier):
        item = self.browser.image_viewer.get_object_item(point)
        time_max = self.browser._imagecontainer.get_meta_data().times[-1]-1
        #print(item,point,item in self._object_items)
        if button == Qt.LeftButton and not item is None:

            coordinate = self.browser.get_coordinate()
            old_class = None
            # remove the item in the current frame and all subsequent frames of the same movie
            if item in self._object_items: 
                point2 = self._object_items[item]
                tpl = (int(point2.x()), int(point2.y()))
                old_class = self._trajectories.get_class_name(coordinate, tpl)

                if old_class == self._current_class:
                    self._trajectories.remove_in_time(coordinate, old_class, tpl)
...
#here instead of adding tpl as coordinates for the newly labelled object we should use the "true" coordinates which are the tracking center of the point but doesn't work
                try:
                    scene = self.browser.image_viewer.scene()
                    items = scene.items(point.x()-1, point.y()-1, 3, 3, Qt.IntersectsItemShape)
                    items = [i for i in items if isinstance(i, HoverPolygonItem)]
                    if len(items) > 0:
                        found_item = items[0]
                    itemX = int(found_item.x())
                    itemY=int(found_item.y())
                    tpl2 = (itemX, itemY)
                    self._trajectories.add(coordinate,
                                           self._current_class, tpl)
                except:
                    exception(self, "Problems saving annotation data...Alice")
                self._object_items[item] = point
...
#            item2.show()
    ...
    def activate(self):
        super(TrackingModule, self).activate()
        self._activate_objects_for_image(True, clear=True)
        self._update_class_table()
    ...
    def deactivate(self):
        super(TrackingModule, self).deactivate()
        self._activate_objects_for_image(False, clear=True)
