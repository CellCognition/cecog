    ...
    OPTIONS = [
      ('tracking',
       [('tracking_regionname', SelectionTrait2(None, [], label='Region name')),
        ('tracking_maxobjectdistance', IntTrait(17, 0, 4000, label='Max object x-y distance')),
        ('tracking_maxtrackinggap', IntTrait(3, 0, 4000, label='Max time-point gap')),
        ('tracking_maxsplitobjects', IntTrait(2, 0, 4000, label='Max split events')),
      ...
      ('event_selection',
       [('tracking_labeltransitions', StringTrait('(1,2)', 200, label='Class transition motif(s)',
                                                  mask='(\(\d+,\d+\),)*\(\d+,\d+\)')),
        ('tracking_backwardrange', FloatTrait(4, -1, 4000, label='Duration [pre]')),
        ('tracking_forwardrange', FloatTrait(16, -1, 4000, label='Duration [post]')),
        ('tracking_backwardlabels', StringTrait('1', 200, label='Class filter [pre]', mask='(\d+,)*\d+')),
        ('tracking_forwardlabels', StringTrait('2', 200, label='Class filter [post]', mask='(\d+,)*\d+')),
