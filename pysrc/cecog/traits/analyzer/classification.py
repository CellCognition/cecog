"""
                           The CellCognition Project
                     Copyright (c) 2006 - 2010 Michael Held
                      Gerlich Lab, ETH Zurich, Switzerland
                              www.cellcognition.org

              CellCognition is distributed under the LGPL License.
                        See trunk/LICENSE.txt for details.
                 See trunk/AUTHORS.txt for author contributions.
"""

__author__ = 'Michael Held'
__date__ = '$Date$'
__revision__ = '$Rev$'
__source__ = '$URL$'

__all__ = ['SectionClassification']

#-------------------------------------------------------------------------------
# standard library imports:
#

#-------------------------------------------------------------------------------
# extension module imports:
#

#-------------------------------------------------------------------------------
# cecog imports:
#
from cecog.traits.config import _Section
from cecog.gui.guitraits import (StringTrait,
                                 BooleanTrait,
                                 SelectionTrait,
                                 )
from cecog.analyzer import (REGION_NAMES_PRIMARY,
                            REGION_NAMES_SECONDARY,
                            )

#-------------------------------------------------------------------------------
# constants:
#
SECTION_NAME_CLASSIFICATION = 'Classification'

#-------------------------------------------------------------------------------
# functions:
#


#-------------------------------------------------------------------------------
# classes:
#
class SectionClassification(_Section):

    SECTION_NAME = SECTION_NAME_CLASSIFICATION

    OPTIONS = [

    ('primary_features',
     [('primary_featurecategory_basicshape',
            BooleanTrait(True, label="Basic shape features")),
      ('primary_featurecategory_intensity',
            BooleanTrait(True, label="Basic intensity features")),
      ('primary_featurecategory_haralick',
            BooleanTrait(True, label="Haralick features")),
      ('primary_featurecategory_stat_geom',
            BooleanTrait(True, label="Statistical geometric features")),
      ('primary_featurecategory_convhull',
            BooleanTrait(False, label="Convex hull features")),
      ('primary_featurecategory_distance',
            BooleanTrait(False, label="Distance map features")),
      ('primary_featurecategory_granugrey',
            BooleanTrait(False, label="Granulometry features")),
      ('primary_featurecategory_moments',
            BooleanTrait(False, label="Moments")),
      ]),

    #old: ('primary_features',
#       [('primary_simplefeatures_texture',
#            BooleanTrait(True, label='Texture features')),
#        ('primary_simplefeatures_shape',
#            BooleanTrait(True, label='Shape features')),
#        ]),

      ('primary_classification',
       [('primary_classification_envpath',
            StringTrait('', 1000, label='Classifier folder',
                        widget_info=StringTrait.STRING_PATH)),
        ('primary_classification_regionname',
            SelectionTrait(REGION_NAMES_PRIMARY[0], REGION_NAMES_PRIMARY,
                           label='Region name')),
        ('primary_classification_annotationfileext',
            StringTrait('.xml', 50, label='Annotation ext.')),
        ]),

#old:      ('secondary_features',
#       [('secondary_simplefeatures_texture',
#            BooleanTrait(True, label='Texture features')),
#        ('secondary_simplefeatures_shape',
#            BooleanTrait(True, label='Shape features')),
#        ]),

    ('secondary_features',
     [('secondary_featurecategory_basicshape',
            BooleanTrait(True, label="Basic shape features")),
      ('secondary_featurecategory_intensity',
            BooleanTrait(True, label="Basic intensity features")),
      ('secondary_featurecategory_haralick',
            BooleanTrait(True, label="Haralick features")),
      ('secondary_featurecategory_stat_geom',
            BooleanTrait(True, label="Statistical geometric features")),
      ('secondary_featurecategory_convhull',
            BooleanTrait(False, label="Convex hull features")),
      ('secondary_featurecategory_distance',
            BooleanTrait(False, label="Distance map features")),
      ('secondary_featurecategory_granugrey',
            BooleanTrait(False, label="Granulometry features")),
      ('secondary_featurecategory_moments',
            BooleanTrait(False, label="Moments")),
      ]),

      ('secondary_classification',
       [('secondary_classification_envpath',
            StringTrait('', 1000, label='Classifier folder',
                        widget_info=StringTrait.STRING_PATH)),
        ('secondary_classification_regionname',
            SelectionTrait(REGION_NAMES_SECONDARY[0], REGION_NAMES_SECONDARY,
                           label='Region name')),
        ('secondary_classification_annotationFileExt',
            StringTrait('.xml', 50, label='Annotation ext.')),
        ]),

      ('collectsamples',
       [('collectsamples',
            BooleanTrait(False)),
        ('collectsamples_prefix',
            StringTrait('',100)),
        ])
      ]
