...
class LDA(BaseEstimator, ClassifierMixin, TransformerMixin):
    ...
    def fit(self, X, y, store_covariance=False, tol=1.0e-4):
        ...
        self.sphering_coef_ = np.dot(self.means_ - self.xbar_, self.scaling)
        self.intercept_ = -0.5 * np.sum(self.sphering_coef_ ** 2, axis=1) + \
                           np.log(self.priors_)

        self.classes = classes
        self.coef_ = self.scaling.T
    ...
    def decision_function(self, X):
        ...
        return np.dot(X, self.sphering_coef_.T) + self.intercept_
    ...
    def transform(self, X):
        ...
        return np.dot(X, self.sphering_coef_[:n_comp].T)
