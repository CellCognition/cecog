...
def dwell_boxplot(data, title=None, xlabel='class label',
    ...
    bp = axes.boxplot(list(data.values()))#, notch=0, sym='+', vert=1, whis=1.5)
    ...
    if ymax not in (-1, None):
        yr[1] = ymax
    axes.set_ylim(yr)
    xlabels = [str(k) for k in list(data.keys())]
    xlabels = [os.linesep.join(textwrap.wrap(str(l), 12)) for l in xlabels]
    axes.set_xticklabels(xlabels, rotation=45, fontsize='small')

    pos = dict((k, v) for k, v in zip(list(data.keys()), axes.get_xticks()))
    # fill boxes with class color
    for box, label in zip(bp['boxes'], list(data.keys())):
        boxX = list()
        boxY = list()
        for j in range(5):
            boxX.append(box.get_xdata()[j])
            boxY.append(box.get_ydata()[j])
            coords = list(zip(boxX, boxY))
        color = cmap(label)
...
def barplot(data,
    ...
    values = [np.average(v) for v in list(data.values())]
    colors = [cmap(k) for k in list(data.keys())]
    width = 2/3
    ...
    top = axes.get_ylim()[1]
    pos = dict((k, v) for k, v in zip(list(data.keys()), ind))
