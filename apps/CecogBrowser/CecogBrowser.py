...
class ImageViewer(StyledFrame):
    ...
    def set_channels(self, channels):
        self.channel_mapping = dict([(k, v) for k,v in channels])

    def update_color_by_name(self, name, color):
        self.channel_mapping[name] = (color.red(), color.green(), color.blue())
        self._update_view()
...
class ColorBox(StyledComboBox):
    ...
    def emit_selection(self):
        self.colorSelected.emit(self._name, self.get_current())
...
class ChannelFrame(StyledSideFrame):
    ...
    def _add_color_box(self, name, color, colors):
        ...
        layout = QVBoxLayout()
        layout.setMargin(0)
        label = StyledLabel(name, frame)
        box = ColorBox(frame, name, color, colors)
        box.colorSelected.connect(self.on_update_color)
        layout.addWidget(label)
        layout.addWidget(box)
        frame.setLayout(layout)
        self.layout.addWidget(frame)
    ...
    def on_update_color(self, name, color):
        print name, color
        self.viewer.update_color_by_name(name, color)
