...
read.screen <- function(dir,filenameLayout, regionName, graph, 
                        fuseClasses=NULL, singleBranch=FALSE) {
  screen <- list()
  screen$dir <- dir
  ...
  for (i in 1:nr) {
    ...
    if (file.exists(path)) {
      filename <- list.files(path, paste(".*",regionName,".*",sep=""))
        ...
        coln = colnames(screen$layout)
        if ('GeneSymbol' %in% coln)
          gene <- rep(screen$layout$GeneSymbol[i],length(filename))
        else
          gene <- rep(screen$layout$Gene.Symbol[i],length(filename))
        if ('OligoID' %in% coln)
          oligoid <- rep(screen$layout$OligoID[i],length(filename))
        else
          oligoid <- rep(screen$layout$siRNA.ID[i],length(filename))
...
plot.transition.graph <- function(hmm, loops=FALSE, type=NULL, filename=NULL,
                                  weights=TRUE)
{
    if (weights)
    {
...
write.hmm.report <- function(screen, prob, outdir, graph,
                             openHTML=TRUE,
                             sortClasses=NULL, 
                             filterClasses=NULL,
                             indices=NULL,
                             ...
                             galleries=NULL)
{
  ...
  for (i in 1:groups) {
    print(sprintf("This is position %d", i))
    if (groupByOligoId) {
    ...
    N.gene <- sum(I)
    print(paste("Generate HMM:", pos.name, " Pos:", str.pos.list, " Group:", i, 
                " Samples:", N.gene))
    ...
    N.gene.old = N.gene
    {
      hmm[[i]] <- hmm.learn(prob[I,,], graph, steps=hmm_em_steps, 
                            initial_emission=hmm_initial_emission)

      Sequence.Raw[I,] <- apply(prob[I,,], c(1,2), which.max)
      if (visualizeDecode) {
        dirHmm <- paste(screen$dir, "..", "hmm", "_visualized",
                        screen$regionName, sep="/")
      ...
      if (write_decode2) {
        ...
        for (f in 1:dim(mcell)[1]) {
          ...
          write.table(Sequence[I,][f,], paste(dirHmm2, mcell$name[f], sep="/"), 
      ...
      plot.transition.graph(hmm[[i]], type="PNG", 
                            filename=paste(outdir_sequences,"/graph__",pos.name,".png",
                                           sep=""), loops=FALSE, weights=TRUE)

      # write HTML
      T1[2,i] <- hwriteImage(paste(rel_sequences,"/graph__",pos.name,".png",sep=""), 
                             link=paste(rel_sequences,"/graph__",pos.name,".png",sep=""), 
                             width=400, height=400)

      if (!is.null(galleries)) {
        T1[3,i] <- hwrite(paste(galleries, 'gallery'), table=T, 
                          link=paste('_gallery', galleries, 
                                     sprintf("%s.jpg", pos.name), sep='/'), 
      ...
      if (!is.null(realign_onset) || !is.null(realign)) {
        ...
        if (is.null(overall_realign)) {
          ...
          overall_realign$id = c(overall_realign$id, 
                                 rep(i, length(realign.starts.nofilter)))
      ...
      if (!is.null(sortClasses)) {
        ...
        I.sort <- rev(sort(apply(occurence, 1, sum), index.return=TRUE, 
                           method="shell")$ix)
        sq <- sq[I.sort,]
      }
      ...
      # prevent empty filenames. this occurs only if the event folder is empty 
      # (no events found), but is behavior
      ...
      write.table(export.data, paste(dirHmm, "/", pos.name, ".txt", sep=""),
                  quote=FALSE, sep="\t", row.names=FALSE, 
                  col.names=c("Trajectory", "Realign"))
      ...
      if (groupByGene) {
        ...
        plot_title = paste(pos.name, " - ", gene.name, " n=", N.gene,"/",
                           N.gene.old, sep="")
      }
      print(plot_title)
      T1[1,i] <- plot_title
      ...
      png(paste(outdir_region,"/",fn[i,1],sep=""), width=1000, height=1000, 
          bg='transparent')
      par(mar=c(0,0,0,0))

      if (N.gene > 1) {
            image(t(sq), col=class.colors.hmm, zlim=c(1,K), xaxt="n", yaxt="n")
            box()
      }
      dev.off()
      ...
      write.table(t(counts.time), paste(dirCounts, "/", pos.name, ".txt", 
                                        sep=""), quote=FALSE, sep="\t", 
      ...
      boxplot(as.data.frame(t(counts.time), optional=TRUE), col=class.colors.hmm, 
              ylim=c(0,max_time), xlab="class", ylab="time [min]")
      title(plot_title)
      dev.off()

      png(paste(outdir_region,"/",fn.b[i,2],sep=""), width=400, height=400)
      barplot(apply(counts.time, 1, mean, na.rm=TRUE), col=class.colors.hmm, 
              ylim=c(0,max_time), xlab="class", ylab="time [min]")
      title(plot_title)
      dev.off()
      ...
      if (!is.null(features)) {
        ...
        for (i in 1:dim(sq)[1]) {
          if (!isPlot) {
            plot(sq.f[i,], xlim=xlim, ylim=ylim, type='l', xaxt='n',
                 xlab='time (min)', ylab='intensity', col=col, xaxs='i', 
                 yaxs='i',las=1)
  ...
  plot.transition.graph(graph, type="PNG", 
                        filename=paste(outdir_sequences, "/graph_prior.png",sep=""),
                        loops=FALSE,weights=FALSE)
  plot.transition.graph(graph, type="PNG", 
                        filename=paste(outdir_sequences,"/graph_prior_loop.png", sep=""),
                        loops=TRUE,weights=FALSE)
  hwrite("Prior Selected Graph Structure",p,heading=3)
  hwriteImage(paste(rel_sequences, "/graph_prior.png",sep=""), p, 
              link=paste(rel_sequences,"/graph_prior.png",sep=""))
  ...
  write.table(countsAll.mean, paste(dir_summary, "/all_mean.txt", sep=""), 
              quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)
  write.table(countsAll.sd, paste(dir_summary, "/all_sd.txt", sep=""), 
              quote=FALSE, sep="\t",row.names=FALSE, col.names=FALSE)
  write.table(countsAll.median, paste(dir_summary, "/all_median.txt", sep=""), 
              quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)
  write.table(TrajectoryNumber, paste(dir_summary, "/all_TrajectoryNumbers.txt", 
              sep=""), quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)
  ...
  print(cnames)
  print(dim(together))
  write.table(together, paste(dir_summary, "/all_together.txt", sep=""), 
              quote=FALSE, sep="\t", row.names=FALSE, col.names=cnames)
